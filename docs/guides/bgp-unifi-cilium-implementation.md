...
| `cilium/app/ocirepository.yaml.j2` | ⚠️ Update version | Change `1.18.5` → `1.19.0-pre.3` |
| `bootstrap/helmfile.d/01-apps.yaml.j2` | ⚠️ Update version | Change `1.18.5` → `1.19.0-pre.3` |

### Required Version Updates

To adopt Cilium 1.19.0-pre.3 (or later), update these files:

**1. `templates/config/kubernetes/apps/kube-system/cilium/app/ocirepository.yaml.j2`:**
```yaml
spec:
  ref:
    tag: 1.19.0-pre.3  # Was: 1.18.5
```
...
releases:
  - name: cilium
    version: 1.19.0-pre.3  # Was: 1.18.5
```
...
## Step 2: Create UniFi FRR Configuration

Create the file `unifi/bgp.conf` in your repository root:
...
! Matches the full VLAN 7 CIDR (192.168.20.0/22)
! =============================================================================
...
1. **`bgp router-id`** - Use the VLAN 7 gateway IP (`192.168.23.254`), not the UDM management IP
2. **`no bgp ebgp-requires-policy`** - **Required** for route exchange without explicit policies
3. **`no bgp default ipv4-unicast`** - Prevents automatic activation (explicit activation in address-family)
4. **Prefix lists** - Must come AFTER the `exit` from `router bgp` section
5. **Neighbor IPs** - Must match the `address` field for each node in your `nodes.yaml` (VLAN 7 IPs)
6. **Prefix list CIDR** - Must match `node_cidr` (`192.168.20.0/22`)
...
2. **CiliumBGPPeerConfig** - Peer connection settings
...
For added security, enable TCP MD5 authentication:
...
**2. Update CiliumBGPPeerConfig Template:**
...
```frr
neighbor TALOS password your-secure-password
```

### Enable ECMP (Multi-Path Routing)

For load balancing across multiple nodes:
...
- [Research: BGP UniFi Cilium Integration](../research/bgp-unifi-cilium-integration.md) - Original research document
