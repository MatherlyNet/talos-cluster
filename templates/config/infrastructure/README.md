...
# Edit cluster.yaml - add proxmox_api_url and proxmox_node to enable infrastructure
# Edit nodes.yaml - add VM specs per node (vm_cores, vm_memory, etc.)

# Generate all configs (including infrastructure secrets)
task configure

# Edit secrets with your credentials
task infra:secrets-edit
# Update: tfstate_username, tfstate_password, proxmox_api_token_* (if enabled)
...
1. Edit `cluster.yaml` for Proxmox settings and VM defaults
...
4. Run `task configure` to regenerate
5. Run `task infra:secrets-edit` to update secret values
...
Infrastructure secrets are **auto-generated** and encrypted during `task configure`, following the same pattern as Kubernetes secrets.

**Template:** `templates/config/infrastructure/secrets.sops.yaml.j2`

The template conditionally includes Proxmox credentials based on `infrastructure_enabled` (when `proxmox_api_url` and `proxmox_node` are set in `cluster.yaml`).
...
# Included when infrastructure_enabled:
proxmox_api_token_id: "..."
proxmox_api_token_secret: "..."
```

**Workflow:**
1. `task configure` - Generates and encrypts `infrastructure/secrets.sops.yaml`
2. `task infra:secrets-edit` - Opens encrypted file in SOPS for editing
...
2. **Enable Proxmox** - Add `proxmox_api_url` and `proxmox_node` to cluster.yaml, then `task configure`
3. **Add Credentials** - Run `task infra:secrets-edit` to add Proxmox API token
